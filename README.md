...
기존 안전빵 앱과 분리된 정적 MVP가 `public/ideal-radar-mvp/`에 있습니다.
배포 후 `/ideal-radar-mvp/` 경로에서 실행되며 안전빵 루트 화면과 API는 변경하지 않습니다.

- 이상형/실제 나 아바타 성별 선택
- 성격 테스트 및 레이더 온보딩
- FIND / FOUND / MUTUAL 테스트 시그널
- 내면 정보 2개 무료 공개
- 3라운드 밸런스게임과 티켓 기반 최종 선택
